    ...
    def edit_one_image(self):
        ...
        for it in range(self.args.n_iter):
            ...
            with torch.no_grad():
                ...
                x_lat_path = os.path.join(self.args.image_folder, f'x_lat_t{self.args.t_0}_it{it}_ninv{self.args.n_inv_step}.pth')
                #     if not os.path.exists(x_lat_path):
                seq_inv = np.linspace(0, 1, self.args.n_inv_step) * self.args.t_0
                seq_inv = [int(s) for s in list(seq_inv)]
                seq_inv_next = [-1] + list(seq_inv[:-1])

                print(f"Reverse process ...")
                x = x0.clone()
                x_lat = x0.clone()
                with tqdm(total=len(seq_inv), desc=f"Inversion process ") as progress_bar:
                ...
                tvu.save_image((x + 1) * 0.5, os.path.join(self.args.image_folder,
                                                        f'1_lat_it{it}_ninv{self.args.n_inv_step}.png'))
